...
Path(val_path).mkdir(parents=True, exist_ok=True)  # Create validation directory
...
val_num = int(total_file_groups * 0.1)   # 10% go to validation
test_num = total_file_groups - train_num - val_num   # Remaining go to test

# Split file groups into train, validation, and test
train_file_groups = file_groups[:train_num]
val_file_groups = file_groups[train_num:train_num + val_num]  # Split the file groups for validation
test_file_groups = file_groups[train_num + val_num:]
...
val_files = [file for group in val_file_groups for file in group]
...
for folder in Path(image_path).iterdir():
    if folder.is_dir():
        ...
        folder_val_files = [f for f in val_files if f.parent.name == folder_name]
        folder_test_files = [f for f in test_files if f.parent.name == folder_name]
        print(f"Gambar '{folder_name}' masuk ke train: {len(folder_train_files)}, validation: {len(folder_val_files)}, test: {len(folder_test_files)}")

# Print details
print('Total gambar: %d' % len(all_files))
print('Gambar yang dipindahkan ke train: %d' % len(train_file_groups))
print('Gambar yang dipindahkan ke validation: %d' % len(val_file_groups))
